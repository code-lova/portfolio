    ...
    <nav className={`${styles.paddingX} w-full flex item-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        ...
        >
          ...
          />
          <p className='text-white text-[18px] font-bold cursor-pointer sm:block hidden'>Ebizo Jeremiah</p>
        </Link> 
        
        <ul className='list-none hidden sm:flex flex-row gap-10'>
           {navLinks.map((link) => (
              <li 
              key={link.id} // Added key attribute here
              className={`${active === link.title ? 'text-white' : 'text-secondary'} hover:text-white text-[18px] font-medium cursor-pointer`} 
              onClick={() => setActive(link.title)} // Added this line
              >
        ...
        <div className='sm:hidden flex flex-1 justify-end items-centr'>
          ...
          <div className={`${!toggle ? 'hidden': 'flex'} p-6 black-gradient absolute top-20 right-0 mx-4 min-w-[140px] z-10 rounded-xl`}>
            <ul className='list-none flex justify-end items-start flex-col gap-4'>
              {navLinks.map((link) => (
                  ...
                  onClick={() => {setToggle(!toggle); setActive(link.title)}} // Added this line
                  >
                    <a href={`#${link.id}`}>{link.title}</a>
                  </li>
              ))}
